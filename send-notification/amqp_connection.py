...
def create_connection(hostName, port=5672, max_retries=12, retry_interval=5):
    ...
    while retries < max_retries:
        try:
            ...
            # url_params = pika.URLParameters("amqp://guest:guest@"+hostName+":"+port+"?heartbeat=3600&connection_attempts=10&retry_delay=1")
...
def check_exchange(channel, exchangename, exchangetype):
    try:    
        channel.exchange_declare(exchangename, exchangetype, durable=True, passive=True) 
            # passive (bool): If set, the server will reply with Declare-Ok if the exchange already exists with the same name, and raise an error if not. 
