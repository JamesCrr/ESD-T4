<template>
    <div>
        <md-table v-model="listings" :table-header-color="tableHeaderColor">
            ...
            <md-table-row v-for="(listing, index) in listings" :key="index">
              ...
              <md-table-cell>
                <md-button :disabled="!listing.status" :class="{ 'md-danger': listing.status }" @click="showConfirmationDialog(listing)">
                  End Auction
                </md-button>
              </md-table-cell>
      ...
      <md-dialog :md-active.sync="confirmDialogActive">
        ...
        <md-dialog-content>Are you sure you want to end this auction?</md-dialog-content>
        <md-dialog-actions>
          <md-button @click="confirmDialogActive = false">No</md-button>
          <md-button class="md-info" @click="closeAuction(selectedListing)">Yes</md-button>
        </md-dialog-actions>
      </md-dialog>
  ...
  const getListingsURL = "http://localhost:9999/getListingsBySeller/3ae1a890-fa30-47c3-ac70-6a282d492b4b";
